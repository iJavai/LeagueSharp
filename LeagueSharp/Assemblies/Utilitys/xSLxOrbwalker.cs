...
// ReSharper disable once InconsistentNaming
    public class xSLxOrbwalker {
        ...
        public static void AddToMenu(Menu menu) {
            ...
            menuMisc.AddItem(
                new MenuItem("orb_Misc_Priority_Unit", "Priority Unit").SetValue(new StringList(new[] {"Minion", "Hero"})));
        ...
        private static void OnDraw(EventArgs args) {
            ...
            if (Menu.Item("orb_Draw_AARange").GetValue<Circle>().Active) {
                Utility.DrawCircle(MyHero.Position, GetAutoAttackRange(),
                    Menu.Item("orb_Draw_AARange").GetValue<Circle>().Color);
            }
            ...
            if (Menu.Item("orb_Draw_Holdzone").GetValue<Circle>().Active) {
                Utility.DrawCircle(MyHero.Position, Menu.Item("orb_Misc_Holdzone").GetValue<Slider>().Value,
                ...
                foreach (
                    ...
                    if (Menu.Item("orb_Draw_Lasthit").GetValue<Circle>().Active &&
                        ...
                        Utility.DrawCircle(minion.Position, minion.BoundingRadius,
                            Menu.Item("orb_Draw_Lasthit").GetValue<Circle>().Color);
        ...
        private static void MoveTo(Vector3 position) {
            int delay = Menu.Item("orb_Misc_Humanizer").GetValue<Slider>().Value;
            if (Environment.TickCount - _lastMovement < delay)
                return;
            _lastMovement = Environment.TickCount;

            int holdAreaRadius = Menu.Item("orb_Misc_Holdzone").GetValue<Slider>().Value;
        ...
        public static double GetAzirAASandwarriorDamage(Obj_AI_Base unit) {
            ...
            double dmg = damagelist[MyHero.Level - 1] + (MyHero.BaseAbilityDamage*0.7);
            if (
                    ...
                    foreach (
                        Obj_AI_Minion minion in
                            ObjectManager.Get<Obj_AI_Minion>()
                                .Where(
                                    minion =>
                                        InSoldierAttackRange(minion) && minion.Name != "Beacon" &&
                ...
                foreach (Obj_AI_Minion minion in from minion in ObjectManager.Get<Obj_AI_Minion>()
                    ...
                    let predHealth =
                        HealthPrediction.LaneClearHealthPrediction(minion,
                            (int) ((MyHero.AttackDelay*1000)*LaneClearWaitTimeMod), FarmDelay(-125))
                    where predHealth >=
        ...
        private static Obj_AI_Base GetBestHeroTarget() {
            ...
            if (MyHero.ChampionName == "Azir") {
                foreach (
                    Obj_AI_Hero enemy in AllEnemys.Where(hero => hero.IsValidTarget() && InSoldierAttackRange(hero))) {
                    double killHits = CountKillhitsAzirSoldier(enemy);
                ...
                foreach (
                    Obj_AI_Hero enemy in
                        AllEnemys.Where(hero => hero.IsValidTarget() && InSoldierAttackRange(hero)).Where(
                            enemy =>
                                mostdmgenemy[0] == null ||
            ...
            return hitsToKill <= 3
                ? killableEnemy : SimpleTs.GetTarget(GetAutoAttackRange(), SimpleTs.DamageType.Physical);
        }
        ...
        private static void CheckAutoWindUp() {
            ...
            if (windUp < 40)
                windUp = 40;
            Menu.Item("orb_Misc_ExtraWindUp").SetValue(windUp < 200
