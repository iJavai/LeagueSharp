...
namespace Assemblies {
    internal class Akali : Champion {
                ...
                foreach (
                    ...
                    if (
                        HealthPrediction.GetHealthPrediction(minion,
                            (int) (E.Delay + (minion.Distance(player)/E.Speed))*999) <
                        player.GetSpellDamage(minion, SpellSlot.Q) &&
                        HealthPrediction.GetHealthPrediction(minion,
                            (int) (E.Delay + (minion.Distance(player)/E.Speed))*999) > 0 &&
        ...
        private void castR() {
            ...
            if (target == null ||
                !target.IsValidTarget(R.Range) && target.Distance(player) < Orbwalking.GetRealAutoAttackRange(player))
                return;
            R.Cast(target, true);
        }

        private void RapeTime() {
            Obj_AI_Hero possibleVictim = SimpleTs.GetTarget(R.Range*2 + Orbwalking.GetRealAutoAttackRange(player),
            ...
            if (rektmate != default(Obj_AI_Hero)) {
                if (player.Distance(rektmate) < 1600 &&
                    player.Distance(rektmate) > Orbwalking.GetRealAutoAttackRange(player))
        ...
        private void RaperinoCasterino(Obj_AI_Hero victim) {
            ...
            foreach (InventorySlot item in player.InventoryItems)
                switch ((int) item.Id) {
                    case 3144:
            ...
            if (W.IsReady() && W.InRange(victim.Position))
                W.Cast(V2E(player.Position, victim.Position, player.Distance(victim) + W.Width - 20));
        ...
        private void Escape() {
            ...
            if (!IsWall(pos) && IsPassWall(player.Position, pos.To3D()) &&
                MinionManager.GetMinions(cursorPos, 800, MinionTypes.All, MinionTeam.NotAlly).Count < 1)
        ...
        private void castREscape(Vector3 position, bool mouseJump = false) {
            ...
            foreach (Obj_AI_Base minion in ObjectManager.Get<Obj_AI_Base>())
                if (minion.IsValidTarget(R.Range, true) && player.Distance(target) < player.Distance(minion) &&
                    minion.Distance(position) < target.Distance(position))
                    if (mouseJump) {
